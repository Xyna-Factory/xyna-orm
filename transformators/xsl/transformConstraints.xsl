...
<xsl:text> INITIALLY </xsl:text><xsl:value-of select="@initially"/></xsl:if> USING INDEX TABLESPACE idx</xsl:when>
...
<xsl:text> CONSTRAINT </xsl:text> <xsl:value-of select="@nnConstrName"/>      
